...
void useParent(AfcPipe &pipe, AfcPipeStream &pstream) 
{

	char *ch = new char[22];
	pipe.read(ch, 21);
	AfcTrace("child is ready - he sent '" << ch << '\'');
	delete[] ch;
...
void useChild(AfcPipe &pipe, AfcPipeStream &pstream, std::string str)
{
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    if (fork.parent())
    {
      ...
      AfcTrace("child terminated normally");
    }
    else // child
    {
      pipe.closeReadFd();
      pstream.closeReadFd();
		...
		AfcTrace("waiting for child to become ready");
